...
def send_to_serial(theta1, theta2, theta3, theta4, port='COM6', baud_rate=115200):
    try:
        # Initialize the serial connection


        # Construct the message with the provided theta values
        t2_int = int(theta2)
        t1_int = int(theta1)  
        t3_int = int(theta3+90)
        t4_int = int(theta4)
...
print_interval = 0.1  # Interval between prints, in seconds
...
while cap.isOpened():
    ...
    if current_time - last_print_time >= print_interval:
        send_to_serial(f_theta1, f_theta2, f_theta3, f_theta4)
